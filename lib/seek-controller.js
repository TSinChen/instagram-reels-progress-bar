...
export class SeekController {
  ...
  attach(video, { hit, track }) {
    ...
    // Re-attaching to the same element does not replay pointerenter, so ask the browser
    // whether the pointer is already inside rather than assuming it is not
    this.hovering = typeof hit.matches === 'function' && hit.matches(':hover');
  }
