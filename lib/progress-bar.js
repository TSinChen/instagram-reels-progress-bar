...
export class ProgressBar {
  constructor(doc = document) {
    ...
    this.parts = {};
    this._visible = false;
    // 設定可能在 mount 之前就送到，先記著，mount 時再套上去
  ...
  /**
   * 套用使用者設定。只改 host 上的 CSS 自訂屬性，不重建任何 DOM，
   * 所以拖曳到一半改設定也不會中斷。
   */
   ...
   * 讓浮層自己裁掉影片的圓角。
   *
   * 浮層是 position: fixed 掛在 body 上，不會被影片容器的 overflow: hidden 裁到，
   * 所以圓角影片的進度條兩端會凸出去。裁在 host 而不是進度條本身，是因為
   * host 有 48px 高，圓角曲線畫得完整；直接套在 3px 高的進度條上，
   * CSS 會依規範把半徑等比縮小，畫出來的弧度是錯的。
   *
   * 沒有圓角時不開 overflow，否則進度條在最左或最右時圓點會被切掉一半。
   */
