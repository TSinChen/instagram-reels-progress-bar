...
export class ProgressBar {
  constructor(doc = document) {
    ...
    // Assigning textContent always replaces the text node, even with an identical string
  ...
  mount(parent = this.doc.body) {
    ...
    // Assembled node by node rather than from an HTML string. The string was a constant, but
    // AMO's linter flags every innerHTML write regardless, and a flag costs a manual review round.
    const div = (className) => {
    ...
    const parts = {
      label: div('label'),
      track: div('track'),
      buffered: div('buffered'),
      played: div('played'),
      handle: div('handle'),
      hit: div('hit'),
    };
    parts.track.append(parts.buffered, parts.played);
    rootEl.append(parts.label, parts.track, parts.handle, parts.hit);
  ...
  render(state) {
    ...
    const label = `${formatTime(state.labelTime)} / ${formatTime(duration)}`;
